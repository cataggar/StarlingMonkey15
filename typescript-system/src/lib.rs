...
fn stat(path: &str) -> Option<DescriptorStat> {
    match find_descriptor(path) {
        Some(descriptor) => match descriptor.stat() {
            Ok(stat) => {
                return Some(stat);
            }
            Err(err) => {
...
impl sys::GuestSystem for System {
    ...
    fn get_directories(&self, path: String) -> Vec<String> {
        // println!("get_directories({:?})", path);
        match find_descriptor(&path) {
            Some(descriptor) => match descriptor.read_directory() {
                Ok(entries) => {
                    let mut dirs = Vec::new();
                    while let Ok(Some(entry)) = entries.read_directory_entry() {
                        if entry.type_ == fs::DescriptorType::Directory {
    ...
    fn read_file(&self, path: String, _encoding: Option<String>) -> Option<String> {
        ...
        if let Some(descriptor) = find_descriptor(&path) {
            ...
            println!("read_file not found: {:?}", path);
        }
        None
    }
    ...
    fn get_file_size(&self, path: String) -> u32 {
        ...
        match stat(&path) {
            ...
            None => {
                println!("get_file_size stat not found: {:?}", path);
                return 0;
            }
        }
    }

    fn write_file(&self, path: String, data: String, write_byte_order_mark: bool) {
        println!(
            "write_file({:?}, {:?}, {:?})",
    ...
    fn read_directory(
        ...
        println!(
            "read_directory({:?}, {:?}, {:?}, {:?}, {:?})",
            path, extensions, exclude, includes, depth
        );
        Vec::new()
    }
